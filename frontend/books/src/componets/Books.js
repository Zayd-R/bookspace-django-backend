    ...
    return(
         ...
         {getPaginatedData().map(book=>{
            return(
                <Card   style={{width: '18rem'}} key={book.id}>
                    {book.volumeInfo.imageLinks ? <Card.Img variant="top" src={book.volumeInfo.imageLinks.smallThumbnail}/>: <Card.Img variant="top" src=''/>}
        <Card.Body>
        ...
        <Card.Text>
          ...
          bulk of the card's content.
           ...
           <Link to={`/books/${book.id}`}><Button variant="primary">Go somewhere</Button></Link>
          </Card.Body>
        </Card>
            )
         })}
     ...
     <div className="pagination">
      ...
      {array.map(item=>{
        return(
             ...
             <Button key={item}
             onClick={changePage}
             className={`paginationItem ${currentPage === item ? 'active' : 'notActive'}`}
             >
                <span>{item}</span>
            </Button> 
